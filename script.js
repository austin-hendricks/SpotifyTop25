...
const clientId = "119fb7a25a794371a48c52b81852bb6b"; // your clientId
const username = "ahendricks12"; // your spotify username
...
async function generatePlaylist() {
	getTopTracks().then((uris) => {
		createPlaylistAndFill(uris);
	});
}
...
async function getTopTracks() {
	...
	let trackURIs = [];
	await fetch(topTracksUrl, {
		headers: topTracksHeaders,
	})
		.then((response) => response.json())
		.then((responseJSON) => {
			responseJSON["items"].forEach((track) => {
...
async function createPlaylistAndFill(trackURIs) {
	const accessToken = localStorage.getItem("access_token");
	const authHeaders = {
		Authorization: `Bearer ${accessToken}`,
	...
	// Create the playlist
	const createPlaylistUrl = `${apiBaseUrl}/users/${username}/playlists`;
	...
	await fetch(addTracksUrl, {
		...
		.then((response) => response.json())
		.then((responseJSON) => {
			window.alert("Success! Now go check your Spotify playlists!");
		});
}
